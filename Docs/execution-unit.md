...
The *execution unit* is the hearth of every CPU, it's the unit that enables the core to do computation on numbers. In the full configuration, it supports two data types: **int** and **float**.

In this CPU component resides four different main units: 
...
Each of them, has a general task to accomplish: 

* Integer Unit do arithmetic and logic computations on *integer numbers*.
* Load Store Unit enable the CPU to access the memory.
* Control Status Registers Unit enable the software to write and read the CPU status.
* Floating Point Unit do arithmetic and logic computations on *floating point numbers*.

Those main units also host other sub-units that do different types of computations. Each of those sub-unit can be pipelined, sequential or combinational: 
...
* **Sequential** sub-units can accept only one operation until they finish their task.
...
All the outputs of the sub-units are 0 until it produces a valid result, this is to *avoid using a large multiplexer* to select the output and big registers. Having 0 on all outputs when the result is not valid enables **OR-ing** all the outputs to produce a single valid signal for each *main unit*. This **requires one single valid result per clock cycles**, otherwise the result will be corrupted, the scheduler will ensure this.

The **instruction packets** carry information of the current operation executed along with the pipeline. It's necessary because of the out of order execution and the interrupt / trap event. The packet fields are: 
...
1  | Is float | Only if *F-extension* is implemented, indicates that the destination register is from the float register file | 
...
To clear the RAW (Read After Write) dependencies caused by the pipelined design, fowarding (or bypassing) is done. If one of the operands of the instruction currently in the *issue stage* match the register destination of an instruction that is going to pass into the *commit stage*, the instruction is issued and the matching operand is swapped with the result of the instruction in the commit stage.

This will grant a nice performance gain since the instruction doesn't need to wait until the result is written back.

One property of the execution pipeline that **has to be satisfied** is: *two or more instructions that have equals register destination cannot be at the same time in the pipeline*. This is because instructions can surpass each other, this example will show this hazard: 
...
By making sure that no matching destination registers are present in the execution stage, the hazard will be eliminated completely. 
...
In this unit, multiplications are performed according to the RISCV **M-extension**, it takes as input two 32 bits number, and output one single 32 bit number. Since a 32 $\times$ 32 multiplication produces a 64 bit number, the output is only one of the halves of the full number. 
...
The load unit is resposable for issuing load requests to the memory controller and elaborating the data received from the memory based on the instruction. There are six types of load, three unsigned and three signed, in general it's possible to load a single byte, an halfword or a full word from the memory. 
...
The following diagram shows the states that the load unit goes through during a request
...
The load unit is resposable for issuing load requests to the memory controller. There are three types of stores for different store width: byte, halfword and word.
