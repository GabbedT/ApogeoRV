...
The cache is a component that increase the performance of the CPU in case of high latency memory accesses, high capacity memories or external memories usually requires a lot of cycles before the data shows up to the CPU. 

While in the configuration *without* the cache, the memory controller is connected directly with the *load store unit*, in the configuration *with* the cache, the memory controller and the load store unit are separated by the cache itself. The LSU will issue request directly to the cache through the memory controller bus and it will assert a bit to switch between requests for the cache and requests directly for memory (for non cachable addresses).

RV32-Apogeo implements an highly parameterizable data cache to get the maximum flexibility of the design. There are three parameters to configure, obviously changing one of those can lead to changes in: area occupied, energy consumption and propagation delay. The parameters are: 

* **Cache Size**: Total number of *data bits* (not metadata!) of the cache. By increasing this, we increase the total capacity of the cache thus the number of addressable blocks.
* **Block Size**: Total number of bits in a cache block. Increasing this will decrease the number of addressable blocks (since each block has now more bits) and add latency on cache load miss because more words need to be transferred from memory.
* **Associativity**: Total number of cache ways, again increasing this will decrease the number of addressable blocks.

The parameters needs to be configured based on the target workload to ensure the maximum efficiency in terms of hit rate and area / energy.


## Cache Address

By changing any of those parameters, the address fields will also change, in this example it's possible to see how the cache address is organized.
...
The address generated by the CPU will have the following values: 

Range | Name | Description |
--- | --- | --- |
[31:12] | Tag         | Used to check the entry against the data in the ways
[11:4]  | Index       | Used to access the cache
[3:2]   | Chip select | Used to access one of the N data banks (one of the words of the block)
...
The cache is not composed only by the memory block. There are also components that control the cache and memory traffic:
...
The cache is directly connected to the **Load Store Unit** which resides in the *Execution unit* in the main pipeline, to the two **store buffers** and the **memory controller**
...
As said, the previous cache configuration example is 4-way, 16KB with a 128 bit (16 bytes) wide data block. 
...
The memory has two ports: **PORT 0** is a **read/write** port and it is used mainly by the store unit controller to write data in cache, load unit controller will use this port to allocate a new cache line in case of a load miss. **PORT 0** usage is arbitrated by appropriate arbiter logic. 

**PORT 1** is a **read** only port and it is exclusively used by load unit controller.

Each port has an appropriate hit logic block connected to the controllers.

A cache way is composed by:

* Status bits
* Tag bits
* Data bits

During a cache read, all the ways are accessed at the same time since the 4 tags need to be compared to the supplied address. During a write, only one way will be written. This is done thanks to way enable bits.
...
To lower the energy consumption and access time, the block is divided into 4 different banks 32 bits wide to match **XLEN**, accessed by the [3:2] bits of the address generated by the CPU. That will add a little bit of decoding logic but lower drastically the memory design complexity and power consumption.
